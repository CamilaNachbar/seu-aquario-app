...
public class AquarioService {
	...
	public List<Aquario> salvarAquario(List<Aquario> aquarios) {
		return aquarioRepository.saveAll(aquarios);
	}

	public void deletarAquario(Aquario aquario) {
		try {
	...
	public void updateAquario(Aquario aquario) {
		try {
			aquarioRepository.save(aquario);
		} catch (Exception e) {
			logger.error("DELTA - Erro ao salvar aquario ");
		}
	}
	...
	public List<Aquario> listByLitragem(String litragem) {
		return aquarioRepository.findByLitragem(litragem);
	}
